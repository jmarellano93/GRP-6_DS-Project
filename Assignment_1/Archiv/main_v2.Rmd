...
The dataset was split into three mutually exclusive subsets: - Training set: used to learn model parameters and preprocessing statistics - Validation set: used for hyperparametertuning and model comparison - Test set: held out for final, unbiased evaluation
...
While the strictly linear baseline successfully satisfied OLS assumptions—evidenced by the flat LOWESS line in the residual plots—it did so by discarding predictors with high explanatory power but complex characteristics (revol_bal, tot_cur_bal). The resulting drop in Adjusted R2 (from 0.44 to 0.31) demonstrates that a linear framework is overly restrictive for this dataset. Furthermore, the persistent sharp boundary observed at the lower edge of the residual cloud (the "floor effect") confirms that even a valid linear model cannot respect the physical constraints of interest rate floors.
...
To optimize the Random Forest for generalization on unseen loan data, we tuned the following key hyperparameters. Using the train and validation-set we conducted an iterative approach to find the best hyperparameters for this modeltype. Older iterations can be found within the commit history on github. The selected values reflect specific characteristics of the LendingClub dataset:
...
### Stability testing (K-fold)

While the Random Forest architecture addresses the bias limitations of the linear model, it introduces a new risk: overfitting. Decision tree ensembles are powerful "memorizers," capable of driving training error near zero if unchecked. Although we implemented min_n = 34 as a regularizer, we must empirically verify that the model is learning generalizable patterns rather than idiosyncrasies of the specific training split.

To assess this, we conducted a stability analysis using 5-fold cross-validation. In this procedure, the combined training and validation data is shuffled and split five times. For each fold, we calculate:

-   Training MSE: How well the model fits the data it has seen.
-   Validation MSE: How well the model predicts held-out data.
-   The Generalization Gap: The difference between the two (MSEval − MSEtrain).

A large, varying gap would indicate high variance (instability), suggesting the model is brittle. A consistent gap indicates a stable model, even if the training error is significantly lower than validation error (a known characteristic of Bagging ensembles).

```{r}
set.seed(123)

# 1. Combine Train and Val to use 85% of data for the CV process
train_val_combined <- bind_rows(train, val)
cv_folds <- vfold_cv(train_val_combined, v = 5)

# 2. Get VALIDATION metrics (Assess on the hold-out part of the fold)
rf_cv <- fit_resamples(
  final_rf_workflow,
  resamples = cv_folds,
...
# Extract Validation MSE
assess_mse_rf <- collect_predictions(rf_cv) %>%
  group_by(id) %>%
  summarise(
...
# 3. Get TRAINING metrics (Assess on the training part of the fold)
# We need a custom function because fit_resamples doesn't return training error
calc_train_mse_rf <- function(wf, folds) {
  purrr::map2_dfr(folds$splits, folds$id, \(split, fold_id) {
    
    # Train on the analysis (training) part of the fold
    fit_obj <- fit(wf, data = analysis(split))
    
    # Predict back on that SAME analysis data to get Training Error
    preds   <- predict(fit_obj, new_data = analysis(split)) %>% 
      bind_cols(analysis(split) %>% select(int_rate))
...
# Run the manual training error calculation
# (Note: This might take a minute as it trains the RF 5 more times)
train_mse_rf <- calc_train_mse_rf(final_rf_workflow, cv_folds)

# 4. Stability Analysis: Combine and Compare
rf_stability <- left_join(train_mse_rf, assess_mse_rf, by = "id") %>%
  mutate(
...
### ![](images/clipboard-813039928.png)

The notable gap between training and validation error confirms that the model is, to some degree, overfitting the training data. This is expected behavior for Random Forests; even with regularization (`min_n = 34`), the underlying decision trees are high-variance learners that aggressively partition the feature space to minimize training error. The model essentially "knows" the training borrowers twice as well as it knows the unseen validation borrowers.

Crucially, while the *gap* is large, the*variance* of that gap is extremely low. The Validation MSE does not fluctuate wildly; it remains stable around 9.37 regardless of how the data is split. This indicates that while the model fits the training data aggressively, it has learned robust, generalizable signals rather than noise. The model is predictably accurate on unseen data.

The stability analysis confirms that the Random Forest is robust. The regularization settings successfully prevented the validation error from behaving erratically. We accept the generalization gap as a trade-off for the model's ability to capture non-linearities (like the "floor effect") that the linear model missed, knowing that its performance on new data will be consistent.

### Performance Evaluation

We evaluate the final Random Forest model (trained on train and val) on the held-out test set to assess its generalization performance.

```{r eval = FALSE}
library(yardstick)
# Retrain on Train + Validation ---
# Combine the data (85% of total)
train_val_combined <- bind_rows(train, val)

# Fit the FINAL model on this combined set
rf_fit_final <- fit(final_rf_workflow, data = train_val_combined)

# --- EVALUATE ON TEST SET ---
# Generate predictions on the test set using the fit
rf_predictions <- predict(rf_fit_final, new_data = test)
...
# Display Results
cat("Random Forest Performance on Test Set (Trained on Train+Val):\n")
print(rf_rmse)
print(rf_mae)
...
Similar to the Random Forest model, we conducted a stability analysis for the XGBoost model using 5-fold cross-validation. This procedure evaluates the model's ability to generalize by comparing training and validation Mean Squared Errors (MSE) across folds.
...
### ![](images/clipboard-1524520572.png){width="663"}

Compared to the previous Random Forest model, which exhibited a much larger generalization gap (averaging approximately 4.34), the XGBoost model demonstrates superior stability. While the Random Forest showed significantly lower training error (\~5.03) compared to its validation error (\~9.37), it indicated a much higher degree of "memorization" or overfitting.

In contrast, the XGBoost model's narrow gap suggests that its learning process is highly efficient at capturing generalizable patterns that exist consistently across the entire dataset. The fact that the Validation MSE remains consistently below 9.0 across all folds indicates that the boosting iterations and regularization parameters (such as learn_rate = 0.06 and loss_reduction = 0.05) have successfully managed the bias-variance tradeoff. This stability provides high confidence that the model will perform reliably when deployed on entirely new, unseen loan applications.

### Performance Evaluation

To assess the predictive capability of the final XGBoost model, we retrained the workflow on the combined Training and Validation sets (85% of the total data) and evaluated its performance against the held-out Test set.

```{r}
library(yardstick)

# --- CRITICAL STEP: Retrain on Train + Validation ---
# Combine the data
train_val_combined <- bind_rows(train, val)

# Fit the FINAL model on this combined set
xgb_fit_final <- fit(final_xgb_workflow, data = train_val_combined)

# --- EVALUATION ON TEST SET ---
# Generate predictions using the NEW fit (xgb_fit_final)
xgb_predictions <- predict(xgb_fit_final, new_data = test)
...
# Print Summary
cat("XGBoost Final Test Metrics (Trained on Train + Val):\n")
print(xgb_rmse)
print(xgb_mae)
...
The model achieved an MSE of 8.80 and an RMSE of 2.967 on the test set. These results indicate that the model explains approximately 54.4% of the variance in interest rates on unseen data (R2=0.544). The relatively small gap between the RMSE (2.967) and MAE suggests that prediction errors are not dominated by extreme outliers but are instead moderately distributed across observations.

Compared to the Random Forest baseline (Test MSE: 9.35, RMSE: 3.06), the XGBoost model shows improved predictive accuracy, confirming the expected benefits of its boosting-based approach.

The Predicted vs. Actual scatter plot for XGBoost shows a strong positive relationship, with the majority of observations aligned along the identity line (y=x). This confirms that the model successfully captures the dominant structure of interest rate determination across the full range of values.

Notably, XGBoost exhibits less pronounced regression-to-the-mean behavior than the Random Forest model. While some underprediction at very high interest rates remains, the deviation from the identity line is reduced. This indicates that the boosting model is better able to adapt to difficult cases that were previously underfit, aligning with its sequential error-correction mechanism where subsequent trees focus explicitly on earlier residual errors.

```{r}
library(yardstick)
library(dplyr)

# Predict on (Train and val) Training Data
train_metrics <- predict(xgb_fit_final, new_data = train_val_combined) %>%
  bind_cols(train_val_combined) %>%
  metrics(truth = int_rate, estimate = .pred) %>%
  mutate(Data = "Training (Train + Val)")

# Predict on Test Data (Reality Check)
test_metrics <- predict(xgb_fit_final, new_data = test) %>%
  bind_cols(test) %>%
  metrics(truth = int_rate, estimate = .pred) %>%
  mutate(Data = "Test")
...
A final comparison between training and test performance confirms the model's superior stability and generalization:

-   Training (Train + Val) RMSE: 2.936

-   Test RMSE: 2.967

While the Random Forest showed a massive Generalization Gap of approximately 4.34 in MSE, the XGBoost model maintains a minimal difference between training and test metrics. This narrow gap confirms that the model has not overfitted the training data and maintains consistent predictive power on entirely new observations.

```{r}
# Extract the underlying model from the FINAL fit
xgb_model_obj <- extract_fit_parsnip(xgb_fit_final)

# Plot top 15 features
vip(xgb_model_obj, geom = "col", num_features = 15, aesthetics = list(fill = "steelblue")) +
  labs(
    title = "Feature Importance (XGBoost)",
    subtitle = "Top 15 predictors by Gain (Final Model)",
    y = "Importance (Gain)",
    x = "Predictor"
  ) +
  theme_minimal()
...
The feature importance plot (based on Gain) highlights the predictors that contribute most to reducing the training loss across boosting iterations. The results indicate that loan term is by far the most influential variable, with a Gain exceeding 0.31. This suggests that the duration of the loan plays a central role in interest rate determination. This is economically intuitive, as longer loan terms typically represent higher risk exposure and are therefore priced with higher interest rates.

The second most important predictor, total revolving credit limit (total_rev_hi_lim), reflects a borrower’s available credit capacity and overall creditworthiness. Its significant contribution indicates that lenders place substantial weight on a borrower’s credit limits when pricing loans. Similarly, recent credit inquiries (inq_last_6mths) emerge as a top factor, consistent with the notion that frequent recent credit applications signal elevated risk. Interestingly, purpose_credit_cardappears among the most influential predictors, indicating that loans issued for credit card refinancing carry systematic pricing information that is distinct from other loan purposes, even when holding other financial variables constant.
...
The distributional comparison shows that the model accurately captures the central mass of the interest rate distribution, with predicted values closely aligned to the actual density in the **10% to 20%** range. However, as noted in the plot subtitle, the predicted distribution exhibits a noticeably narrower spread, indicating reduced variance.

In particular, the "Actual" distribution (gray) shows a secondary peak near the lower bound and a longer tail at the high end which the "Predicted" distribution (green) smooths over. Extreme high interest rates (above 25%) are underrepresented in the predictions. This behavior reflects a well-regularized model that prioritizes stable average performance and minimizes overall prediction error (MSE), at the cost of smoothing rare extreme outcomes. Such variance reduction is consistent with the observed regression-to-the-mean behavior and is typical for tree-based ensemble methods optimized for RMSE.
...
A central takeaway from this iterative process is that data preprocessing and diagnostic analysis proved to be as critical as the choice of model itself. The extensive cleaning, transformation, and consistent encoding steps were the foundation that allowed for fair evaluation and prevented data leakage. In particular, the automated cleaning pipeline ensures that median imputation values and categorical levels computed during training are saved within the recipe object. This is essential for reproducibility, as unseen data—which may consist of a single row—cannot provide its own median or mode for imputation; instead, it must be transformed using the exact parameters derived from the training distribution.
...
```{r}
# Combine the datasets currently in memory
full_data_memory <- bind_rows(train, val, test)
message("Combined dataset dimensions: ", paste(dim(full_data_memory), collapse = " x "))

# Define the Model Specification (Your winning config)
final_xgb_spec <- boost_tree(
  trees          = 500,
  tree_depth     = 5,
  min_n          = 2,
...
# Fit on the FULL combined data
message("Fitting final model on Train + Val + Test...")
final_production_fit <- fit(final_workflow, data = full_data_memory)

# Save the final model
saveRDS(final_production_fit, "final_xgb_model_FULL.rds")
message("Success! Final production model saved.")
```
