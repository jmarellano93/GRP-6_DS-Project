...
# ------------------------------------------------------------------------------
# Section 8: MODEL TRAINING WITH ROBUST CLASS WEIGHTS
# ------------------------------------------------------------------------------

# SCRIPT ARCHITECTURE SUMMARY: SECTION 8
#
# 8.1: STRATEGIC CLASS WEIGHTING (AGGRESSIVE DAMPING)
# Reason: The dataset is heavily imbalanced (most people pay back loans). 
#         Standard "Inverse Frequency" weighting creates weights that are too 
#         large for the minority class, forcing the model to hallucinate errors 
#         and destroying overall accuracy. 
# Function: 
#         1. Calculates base weights using inverse frequency.
#         2. Applies "Aggressive Damping" (Power 0.1): This mathematically 
#            "squashes" the weights, bringing them closer to 1.0. 
#         3. Effect: It nudges the model to pay attention to defaults *without* #            ignoring the majority class, specifically tuning the architecture 
#            to prioritize high overall accuracy (>90%) rather than pure recall.
#
# 8.2: LONG-HORIZON TRAINING WITH ADAPTIVE CALLBACKS
# Reason: Because we lowered the learning rate (0.0001) in Section 7 to ensure 
#         stable learning, the model now requires significantly more time (epochs)
#         to converge. We need automated supervisors to manage this long process.
# Function: 
#         1. Early Stopping (Patience=100): The "Safety Net." If the model stops 
#            improving for 100 epochs, it stops training and restores the best 
#            weights, preventing wasted time and overfitting.
#         2. Reduce LR on Plateau (Patience=50): The "Fine Tuner." If accuracy 
#            stalls, it halves the learning rate, allowing the model to make 
#            micro-adjustments to escape local minima.
#         3. Batch Size (128): Uses a moderate batch size (reduced from 1024) 
#            to increase the frequency of weight updates per epoch, helping the 
#            model settle into a more generalized solution.

# ==============================================================================
# 8.1: Robust Class Weights Calculation
# ==============================================================================
cat("\n=== TRAINING WITH AGGRESSIVE DAMPING (>90% Accuracy Target) ===\n")
total_count <- nrow(train_baked)
possible_classes <- 0:7
class_counts <- table(factor(train_baked$target_class, levels = possible_classes))

# NOTE: Aggressive Damping
damping_power <- 0.1 
...
cat(">> Aggressively Dampened Weights:\n")
...
# Since we are learning slowly, we need to give the model more time to show results.
callbacks <- list(
  callback_early_stopping(
    monitor = "val_sparse_categorical_accuracy", 
    mode = "max", 
    patience = 100,             # Allows 100 epochs of no improvement.
...
history <- model %>% fit(
  ...
  # Smaller batches mean more updates per epoch, usually resulting in a 
  # more granular, slower convergence curve.
  batch_size = 1024, 
