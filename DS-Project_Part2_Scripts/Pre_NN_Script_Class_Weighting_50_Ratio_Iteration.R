...
# ==============================================================================
# Setup PDF Output for Visualizations
# ==============================================================================
...
# FINDING: The target variable 'status' shows severe class imbalance, with '0' (1-29 days past due) being the overwhelming majority. This necessitates resampling strategies.
...
# FINDING: 'AMT_INCOME_TOTAL' exhibits a heavy right-skew (Pareto distribution), justifying log-transformation. Outliers are present in 'CNT_CHILDREN', requiring capping.
for(col in num_cols) {
...
if("NAME_EDUCATION_TYPE" %in% names(df_clean)) {
  ...
  edu_levels <- c("Lower secondary", "Secondary / secondary special", "Incomplete higher", "Higher education", "Academic degree")
...
print(miss_counts[miss_counts > 0])
# FINDING: The Missingness Map is now blank, confirming that all missing values (including the structural ones in Occupation) have been successfully imputed or handled.
...
# FINDING: The final binary TARGET variable preserves the class imbalance observed in the raw data (approx 98% Good vs 2% Bad), confirming the need for SMOTE in the modeling pipeline.
...
# FINDING: Distributions of numeric variables after cleaning. Note that outliers (like 365243 in DAYS_EMPLOYED) have been removed/handled, making the ranges physically possible.
for(col in num_cols) {
...
# FINDING: The correlation matrix reveals a high correlation (~0.88) between CNT_CHILDREN and CNT_FAM_MEMBERS, supporting the decision to drop CNT_FAM_MEMBERS to reduce multicollinearity.
if(ncol(num_df) > 1) {
...
# Visualizes the overlap between Income Type and Missing Occupation
# FINDING: This heatmap reinforces the MNAR finding: the missingness in Occupation is almost perfectly correlated with the 'Pensioner' Income Type.
p_heatmap <- df %>%
  ...
  geom_text(aes(label = scales::percent(Prop, accuracy = 1)), color = "black") +
  labs(title = "Structural Missingness: Occupation vs Income",
       subtitle = "Pensioners have nearly 100% missing occupation data",
       x = "Income Type", y = "Occupation Status", fill = "Proportion") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

print(p_heatmap)

# Interaction Risk Plot: Family Status x Housing Type
# We filter to groups with > 50 observations to avoid noise
# FINDING: Interaction effects are visible. For example, 'Municipal Apartments' show higher default risk, particularly when interacting with specific Family Statuses, suggesting non-linear risk pockets.
p_interaction <- df_clean %>%
  ...
  labs(title = "Risk Interaction: Family Status vs Housing",
       subtitle = "Heatmap of Default Rates (Darker/Purple = Higher Risk)",
       x = "Housing Type", y = "Family Status", fill = "Default Rate") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
# FINDING: The violin plot shows the density of Income for Good vs Bad targets. The similarity in shape suggests that Income alone is not a perfect separator, highlighting the need for a multivariate Neural Network approach.
p_violin <- ggplot(df_clean, aes(x = TARGET, y = AMT_INCOME_TOTAL, fill = TARGET)) +
  ...
  labs(title = "Income Distribution by Target Class (Log Scale)",
       subtitle = "Violin plot reveals density shape differences",
       x = "Target (0=Good, 1=Bad)", y = "Annual Income (Log Scale)") +
  theme_minimal()
...
# --- Phase 6: Municipality Risk & Age vs. Employment Density ---

# Risk Stratification: Default Rate by Housing Type
# We calculate the mean target (0/1) to get the rate
# FINDING: This plot isolates 'Municipal apartment' residents as a high-risk segment with a default rate (~3.9%) nearly double the population average (~2.0%).
p_risk_housing <- df_clean %>%
  ...
  labs(title = "Risk Signal: Default Rate by Housing Type",
       subtitle = "Municipal Apartments show nearly 2x the average default risk",
       x = "Housing Type", y = "Default Rate") +
  theme_minimal()

print(p_risk_housing)

# Feature Engineering Validation: Age Distribution by Work Status
# FINDING: The density plot validates the feature engineering: the 'Not_Working' flag (derived from the 365243 anomaly) effectively captures the older population (Pensioners), while 'Working' covers the younger demographic.
p_employ_age <- ggplot(df_clean, aes(x = AGE, fill = EMPLOYMENT_STATUS_FLAG)) +
  geom_density(alpha = 0.5) +
  labs(title = "Validation of Employment Flag",
       subtitle = "'Not_Working' captures the older population (Pensioners) as expected",
...
# ==============================================================================
# [NEW] Export Final Cleaned Dataset
# ==============================================================================
# Define the output directory (reusing the path from the plot step)
...
# ==============================================================================
# Module 5: Data Preparation (No Resampling)
# ==============================================================================
# STRATEGY CHANGE:
# We do NOT use step_smote here. We prepare the raw, imbalanced training data directly.
# The class imbalance will be handled by the 'class_weight' parameter in Keras.

cat("\n================================================================\n")
cat(" MODULE 5: PREPROCESSING (NO SMOTE - COST SENSITIVE PREP)\n")
cat("================================================================\n")
...
base_recipe <- recipe(TARGET ~., data = train_raw) %>%
  ...
  step_dummy(all_nominal_predictors(), one_hot = TRUE) %>% # One-hot immediately
  step_zv(all_predictors()) %>%
  step_range(all_numeric_predictors(), min = 0, max = 1)   # Min-Max Scaling
...
# --- 5.5 Convert to Keras format and Save ---
output_dir <- "C:/Users/John Arellano/RstudioProjects/GRP-6_DS-Project/DS-Project_Part2_Scripts/Saved_Outputs/Pre_NN_Script_Weighting_50_Iterations"
...
saveRDS(train_keras, file.path(output_dir, "train_tensor_imbalanced.rds"))
saveRDS(val_keras, file.path(output_dir, "val_tensor.rds"))
saveRDS(test_keras, file.path(output_dir, "test_tensor.rds"))

cat(">> Data Processed and Saved. No SMOTE applied.\n")

# ==============================================================================
# Module 6 & 7: Iterative Neural Network Training (WEIGHT ITERATION)
# ==============================================================================
# CHANGE: Instead of iterating through files, we iterate through WEIGHT VECTORS.

cat("\n================================================================\n")
cat(" MODULE 6 & 7: CLASS WEIGHT OPTIMIZATION LOOP\n")
cat("================================================================\n")

library(reticulate)

# --- Load Data ---
train_data <- readRDS(file.path(output_dir, "train_tensor_imbalanced.rds"))
...
# --- Helper Function: Comprehensive Metrics ---
calculate_comprehensive_metrics <- function(y_true, y_pred_prob, threshold = 0.5) {
  ...
  return(list(MCC=MCC, Balanced_Acc=Balanced_Acc, Precision=Precision, Recall=Recall))
}

# --- Define Parameterized Model Builder ---
...
# --- Define 50 Weight Ratios ---
# Range from 1 (No Weighting) to 50 (Extreme Weighting for Minority Class)
weight_ratios <- seq(1, 50, length.out = 50)
...
cat("\nBeginning 50 Iterations of Class Weight Optimization...\n")

for (i in seq_along(weight_ratios)) {
  
  w_ratio <- weight_ratios[i]
  
  # Define Class Weights Dictionary
  # Majority class (0) gets 1.0, Minority class (1) gets the loop ratio
  current_weights <- list("0" = 1.0, "1" = w_ratio)
  
  model <- build_model(input_dim, units_1 = 128, dropout_1 = 0.4)
  ...
  history <- model$fit(
    ...
    class_weight = current_weights, # <--- KEY CHANGE
  ...
  results_df <- rbind(results_df, data.frame(
    ...
    Optimal_Threshold = best_thresh_local,
    MCC = best_metrics$MCC,
    Balanced_Acc = best_metrics$Balanced_Acc
  ))
  
  if(i %% 5 == 0) {
    cat(sprintf("[%d/50] Weight Ratio %.1f | Best Thresh: %.2f | MCC: %.4f\n", 
                i, w_ratio, best_thresh_local, best_metrics$MCC))
  }
  
  rm(model, history, val_probs)
  gc(verbose = FALSE)
}

# --- Save Optimization Log ---
write.csv(results_df, file.path(output_dir, "weight_optimization_log.csv"), row.names = FALSE)

# ==============================================================================
# Module 7.5: Architecture Tuning (Using Best Weight)
# ==============================================================================

cat("\n================================================================\n")
cat(" MODULE 7.5: ARCHITECTURE GRID SEARCH (ON BEST WEIGHT)\n")
cat("================================================================\n")

# 1. Identify Winner
best_run <- results_df %>% arrange(desc(MCC)) %>% slice(1)
winning_weight <- best_run$Weight_Ratio
cat(sprintf(">> Best Class Weight Found: %.2f (MCC: %.4f)\n", winning_weight, best_run$MCC))
...
for(i in 1:nrow(hyper_grid)) {
  ...
  # Train with WINNING WEIGHTS
  history <- model_tune$fit(
    x = train_x_np, y = train_y_np,
    epochs = 20L, batch_size = 512L,
    validation_data = list(val_x_np, val_y_np),
    class_weight = list("0" = 1.0, "1" = winning_weight), # Fixed Winner
    callbacks = cb, verbose = 0L
  )
  ...
  cat(sprintf("   [%d/%d] Units:%3d | Drop:%.1f | LR:%.4f || MCC:%.4f\n", 
              i, nrow(hyper_grid), u, d, lr, best_mcc_grid))
...
cat(">> CHAMPION CONFIGURATION:\n")
cat(sprintf("Weight: %.2f | Units: %d | Drop: %.1f | LR: %.4f | Thresh: %.2f\n", 
            winning_weight, final_units, final_drop, final_lr, final_thresh))
...
final_model$fit(
  ...
  batch_size = 512L,
  validation_data = list(val_x_np, val_y_np),
  class_weight = list("0" = 1.0, "1" = winning_weight),
  callbacks = cb, verbose = 0L
)
...
final_model$save(file.path(output_dir, "best_model_weighted_final.keras"))
