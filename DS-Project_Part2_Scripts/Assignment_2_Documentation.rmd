...
#### Computed Weights: The "Aggressive" Approach
...
#### Manual Tuning: The "Balanced" Approach
...
Hyperparameter Roles The grid search for AdamW focused on three interaction effects:
...
![](images/clipboard-1848757617.png){width="330"}

![](images/clipboard-957607625.png){width="457"}
...
# 5. Model stability and comparison
...
## 5.1. Stability Testing: Post-Hoc K-Fold Cross-Validation
...
```{r model_comparison, echo=FALSE, warning=FALSE}
...
## 5.2. Generalization on the Test Set

The ultimate measure of a predictive model is its performance on data it has never encountered. Throughout the entire project (Chapters 2 and 3), a dedicated 15% Test Set was locked away and strictly excluded from all training, validation, and threshold tuning processes.

To select the final winner, we execute the following protocol: - To maximize the learning signal, we merge the previous Training and Validation sets into a single, comprehensive dataset. - The normalization scaler is re-fitted on this full training set to ensure the best possible feature distribution. - Both the AdamW and SGD Momentum models are retrained on the full dataset and then evaluated on the Hold-Out Test Set.

This final comparison provides an unbiased estimate of real-world performance. The model that achieves the superior balance of Generalization Accuracy and Risk Detection (Class 3 Recall) on this never-before-seen data will be designated as the final production engine.
