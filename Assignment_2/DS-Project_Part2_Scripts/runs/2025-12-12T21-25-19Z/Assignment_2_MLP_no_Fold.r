...
FLAGS <- flags(
  ...
  # Architecture (Small Model for SGD Test)
  flag_integer("units1", 1024),
  flag_integer("units2", 512),
  flag_integer("units3", 0),     
  
  # Optimization
  flag_numeric("learning_rate", 4),
  flag_integer("batch_size", 49000),
...
# Note: No Early Stopping (We are testing the "Long Run" stability)
history <- model %>% fit(
  x_train, y_train,
  batch_size = FLAGS$batch_size,
  epochs = FLAGS$epochs,
  verbose = 1, # Keep verbose to monitor progress
