...
```
The variables tot_coll_amt, tot_cur_bal, and total_rev_hi_lim represent a borrower’s total collection amount, total current balances, and total revolving credit limit. These three features describe the borrower’s overall credit utilization profile, which is essential for assessing creditworthiness.
In the cleaned dataset, 63'251 rows had all three of these variables missing simultaneously.
Such rows were removed because:
- They contain no usable information about the borrower’s credit balance.
- With all three fields missing, the borrower’s credit exposure cannot be approximated or imputed reliably.
- The missingness is structural, not random.
- Borrowers with all three fields missing typically reflect incomplete or corrupted credit bureau data, not normal variation.
- Imputation is not appropriate when the entire feature set is missing.
- Imputing values would inject arbitrary numbers into critical financial variables, degrading model validity.
- They represent a very small portion of the overall dataset, so dropping them does not meaningfully reduce sample size or introduce bias.
For these reasons, rows where tot_coll_amt, tot_cur_bal, and total_rev_hi_lim were all NA were excluded from further analysis.
...
dim(cleaned_data) # 734'905 rows and 37 columns now
```
...
```{r}
extreme_dti <- show_extreme_values(na.omit(cleaned_data), "dti", n = 10)
...
DTI values > 100% are implausible for approved consumer loans and contradict typical underwriting standards, where DTIs above 60–70% are rarely accepted. The fact that the associated int_rate values for these rows are not exceptionally high further suggests that these DTIs do not reflect real borrower risk but are instead the result of data errors or import inconsistencies. Therefore, DTI values above 100% were set to NA for later imputation if desired.
...
suspicious_revol_bal_rows # 3'265 rows
...
suspicious_revol_util_rows # 365'212 rows
...
```
We still see an outlier at tot_coll_amt. Which we will set to NA
```{r}
cleaned_data <- cleaned_data %>%
  mutate(tot_coll_amt = ifelse(tot_coll_amt > 50000, NA, tot_coll_amt))
```

Furthermore there are other columns to drop, which aren't useful for modelling or have too many missing values. These columns are:
- id
- member-id
- revol_util: can be calculated with revol_bal and total_rev_hi_lim anyway
```{r}
cleaned_data <- cleaned_data %>%
  select(-c(id, member_id, revol_util, mths_since_last_delinq, mths_since_last_major_derog, delinq_2yrs))
```
...
For this project, the dataset consisting of 734,905 rows is divided into three independent subsets:
- Training set (70%) → approx. 514,434 rows
- Validation set (15%) → approx. 110,236 rows
- Test set (15%) → approx. 110,236 rows
This partitioning provides a clear and interpretable workflow while preserving a large training set for model development.

#### Why a simple splitting technique
A single split into training, validation, and test sets offers several practical advantages for this assignment:
1.  Clarity and interpretabilit
Each dataset serves a distinct purpose:
- the training set is used for fitting the models and running cross-validation,
- the validation set is used for hyperparameter tuning, feature selection, and model comparison,
- the test set is used once at the end to measure final model performance. This transparency aligns well with the educational objective of explaining each modelling decision.
2. Large dataset removes instability concerns
With 734k rows, a single split provides sufficiently large training, validation, and test samples. Variance from resampling is not a major concern with this sample size.
3. Supports clear explanation of modelling choices
The project requires interpreting and justifying the effects of:
- different imputation strategies,
- different feature selection decisions, model hyperparameters.
A single split makes these stages easy to isolate and explain.
4. Avoids unnecessary complexity
A simple split avoids the computational burden and conceptual overhead of nested cross-validation, which is not required for the learning goals of this project.

#### Why no nested loop
In theory, nested cross-validation represents the most statistically rigorous approach.
It separates:
- the outer loop → unbiased performance estimation
- the inner loop → model selection, feature selection, imputation choice, hyperparameter tuning
Benefits of Nested CV
- Provides the most unbiased estimate of generalization performance
- Prevents any contamination between:
  - model selection
  - preprocessing selection
  - performance estimation
- Automatically incorporates data-dependent preprocessing (imputers, scalers, feature selection) into the evaluation loop
- Ideal for scientific research, regulated environments, or small datasets
However, nested CV also has drawbacks in this assignment context
Extremely high computational cost
Running nested 5×5 CV on 734k rows would require fitting models dozens of times, which is impractical.
Makes explainability harder
The assignment requires demonstrating:
why a specific imputation method was chosen,
how features were selected,
how hyperparameters influence model behaviour.
In nested CV, all these steps are embedded inside opaque inner loops, making it much harder to document and interpret each decision.
Overkill for a large dataset
Nested CV is most useful when data is scarce.
With more than 500k rows in the training set, the benefit over a single validation split is minimal.
Not aligned with the pedagogical goal
The purpose of this project is to learn the reasoning behind model choices, not to maximize statistical purity at any cost.
A simpler split supports clear step-by-step analysis.


### Imputation
Choose imputation method with ONLY Train dataset and make a CV within Train dataset to choose the best one
after you fit set imputation method on the entire TRAINING set

fit imputer on train and apply to val
fit imputer on train and val and apply to test

explain that normally one would do data checks again, but we skip this step for brevity
