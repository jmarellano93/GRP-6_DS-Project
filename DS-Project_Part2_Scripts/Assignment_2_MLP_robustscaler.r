...
FLAGS <- flags(
  ...
  flag_integer("patience", 200)    # Early stopping patience
)
...
continuous_cols <- colnames(x_train)[apply(x_train, 2, function(x) !all(x %in% c(0, 1)))]
...
x_val[, continuous_cols] <- sweep(x_val[, continuous_cols, drop=FALSE], 2, train_medians, "-")
...
list(
  ...
  epochs_trained = length(history$metrics$loss)
)
