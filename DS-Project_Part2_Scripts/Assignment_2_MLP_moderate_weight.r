# ==============================================================================
# STEP 2 (VARIANT C): MODERATE CLASS WEIGHTS
# 
# CHANGE LOG:
# 1. Removed Data Balancing (Oversampling/SMOTE). Data remains untouched.
# 2. Implemented 'Moderate Class Weighting' in the fit() function.
#    - Formula: sqrt(Total / (N_Classes * Count))
#    - Effect: Penalizes minority errors significantly, but not aggressively.
# ==============================================================================

# Assignment 2 MLP (Multi-Layer Perceptron)
# Supports Variable Depth, Robust Scaling, and Class Weights
...
cat("Starting", k, "-Fold Cross-Validation (With Moderate Weights)...\n")
...
for(i in 1:k){
  ...
  # --- NEW: STEP 2 - CALCULATE MODERATE WEIGHTS -----------------------
  ...
  # 2. Calculate Standard Inverse Frequency Weights
  # Formula: Total / (n_classes * count)
  std_weights <- total_count / (n_classes * class_counts)
  
  # 3. Apply Smoothing (Square Root) -> "Moderate" Weights
  # This reduces the gap between large and small weights.
  mod_weights <- sqrt(std_weights)
  
  # 4. Convert to Named List (Required by Keras)
  class_weights_list <- as.list(mod_weights)
  names(class_weights_list) <- as.character(0:7)
  
  cat("  Raw Weights:      ", round(as.numeric(std_weights), 1), "\n")
  cat("  Moderate Weights: ", round(as.numeric(mod_weights), 1), "\n")
  # --------------------------------------------------------------------
...
if(gap > 0.05) {
  ...
  cat("WARNING: Method too weak. Try stronger weights (remove sqrt) or Oversampling.\n")
} else {
