...
# ------------------------------------------------------------------------------
# OPTION 4: CLASS WEIGHTING (Data remains unbalanced)
# Pros: No synthetic noise. Cons: Model convergence can be tricky.
# Note: For this option, we do NOT change the recipe. We calculate weights for Keras.
# ------------------------------------------------------------------------------
final_recipe <- base_recipe
strategy_name <- "Class_Weighting"

# Calculate Class Weights for Module 6 (Model Training)
# Formula: n_samples / (n_classes * n_samples_j)
total_count <- nrow(train_raw)
class_counts <- table(train_raw$TARGET)
weight_0 <- total_count / (2 * class_counts["0"])
weight_1 <- total_count / (2 * class_counts["1"])
class_weights_list <- list("0" = weight_0, "1" = weight_1)
cat("\n[Option 4] Calculated Class Weights for Keras:\n")
