# ==============================================================================
# REPORT VERSION: UPSAMPLING STRATEGY
...
cat("\n================================================================\n")
cat(" MODULE 5: PREPROCESSING PIPELINE (UPSAMPLING STRATEGY)\n")
cat("================================================================\n")

# --- 5.1 Stratified Data Splitting ---
set.seed(123)

#... (Same logic as before for splitting)...
...
# --- 5.1b Calculate Capping Thresholds ---
#... (Same capping logic)...
if("AMT_INCOME_TOTAL" %in% names(train_raw)) inc_cap <- quantile(train_raw$AMT_INCOME_TOTAL, 0.99, na.rm = TRUE) else inc_cap <- Inf
if("CNT_CHILDREN" %in% names(train_raw)) child_cap <- quantile(train_raw$CNT_CHILDREN, 0.995, na.rm = TRUE) else child_cap <- Inf
if("CNT_FAM_MEMBERS" %in% names(train_raw)) fam_cap <- quantile(train_raw$CNT_FAM_MEMBERS, 0.995, na.rm = TRUE) else fam_cap <- Inf
