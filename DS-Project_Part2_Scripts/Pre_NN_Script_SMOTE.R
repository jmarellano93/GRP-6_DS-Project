...
# --- 5.1 Stratified Data Splitting (70% Train, 15% Val, 15% Test) ---
set.seed(123) 
...
# RIGOR: Calculating the 99th percentile on Train Set to avoid Leakage.
...
base_recipe <- recipe(TARGET ~., data = train_raw) %>%
  ...
  # 3. Winsorization (Applying Training Caps to all data)
  step_mutate(
  ...
  # 4. Log Transformation (Applied AFTER Capping)
  # Log-normalizes income to fix skewness
  step_log(AMT_INCOME_TOTAL, offset = 1) %>%
  
  # 5. Encoding (One-Hot)
  step_dummy(all_nominal_predictors(), one_hot = TRUE) %>%
  
  # 6. Zero Variance Filter
  step_zv(all_predictors()) %>%
  
  # 7. Scaling (Z-Score Standardization)
  # CHANGED: step_normalize (Mean 0, SD 1) is better for ReLU networks than MinMax.
...
final_recipe <- base_recipe %>%
  step_smote(TARGET, over_ratio = 1) # Generate synthetic examples until 1:1 balance
strategy_name <- "SMOTE"
...
# 1. PREP: Calculate statistics (medians, means, SDs) on TRAINING data only.
trained_recipe <- prep(final_recipe, training = train_raw)

# 2. BAKE: Apply those transformations to Train, Validation, and Test.
train_processed <- bake(trained_recipe, new_data = NULL) # NULL = Use training data
...
cat("Processed Val Shape:   ", dim(val_processed), "\n")

# Check 2: Scaling Verification (Standardization Check)
cat("\nScaling Check (Should be Mean ~0, SD ~1):\n")
summary(train_processed %>% select(contains("AGE"), contains("INCOME")))

# Check 3: Imbalance Check
