...
# 5.5.3: Scale ONLY numeric features (Skipping ID and Target)
# FIX: explicitly select numeric columns to avoid crashing on 'ID'
features_to_scale <- data %>%
  select(where(is.numeric)) %>%
  names() %>%
  setdiff("target_class") # Exclude target from scaling

cat(sprintf(">> Scaling %d numeric features (Skipping ID)...\n", length(features_to_scale)))

data[features_to_scale] <- lapply(data[features_to_scale], min_max_norm)

# QA Check
if(any(is.na(data[features_to_scale]))) {
  cat(">> WARNING: NAs produced during scaling. Check for constant columns.\n")
} else {
  cat(">> SUCCESS: Features scaled to [0,1].\n")
}
...
# ------------------------------------------------------------------------------
# SECTION 7: SIMPLIFIED PREPROCESSING (CORRECTED)
# ------------------------------------------------------------------------------

cat("\n=== SIMPLIFIED PREPROCESSING FOR >90% ACCURACY ===\n")

# FIX 1: Use 'data' (processed), not 'df_clean' (snapshot), to keep feature selection.
# Check if ID was dropped in Section 5.5; if so, recover it temporarily for splitting.
if(!"ID" %in% names(data)) {
  # We assume row order is preserved (safe if no shuffling occurred since snapshot)
  data$ID <- eda_snapshot$ID
}

# Define Modeling Data (Keep ID for now)
df_model <- data %>% 
  select(-any_of(c("status", "TARGET", "Risk_Label", "Is_Serious_Delinquency")))

# FIX 2: Perform Split BEFORE removing ID
# ------------------------------------------------------------------------------
cat(">> Performing ID-Based Split (Preventing Data Leakage)...\n")
all_ids <- unique(df_model$ID) # ID exists here now
...
# 15% Validation (Half of remaining), 15% Test (Rest)
val_ids <- sample(remaining_ids, size = 0.50 * length(remaining_ids))
test_ids <- setdiff(remaining_ids, val_ids)

# Filter Dataframes by ID and THEN drop the ID column
train_df <- df_model %>% filter(ID %in% train_ids) %>% select(-ID)
val_df   <- df_model %>% filter(ID %in% val_ids)   %>% select(-ID)
test_df  <- df_model %>% filter(ID %in% test_ids)  %>% select(-ID)

cat(sprintf("Data Split by ID: Train=%d, Val=%d, Test=%d (Rows)\n", 
            nrow(train_df), nrow(val_df), nrow(test_df)))

# Recipe: 
# Since 'data' is already scaled/encoded from Section 5, this recipe mostly 
# just acts as a format converter to Matrix, which is safe.
simple_recipe <- recipe(target_class ~ ., data = train_df) %>%
  # Note: step_dummy will effectively do nothing if data is already numeric, which is fine.
  step_dummy(all_nominal_predictors(), one_hot = TRUE) %>%        
  step_range(all_numeric_predictors(), min = 0, max = 1) %>%      
  step_rm(target_class) %>%                                       
  prep(training = train_df)

# Transform
train_baked <- bake(simple_recipe, new_data = train_df)
val_baked   <- bake(simple_recipe, new_data = val_df)
test_baked  <- bake(simple_recipe, new_data = test_df)

# Convert to Matrix
x_train <- data.matrix(train_baked)
x_val   <- data.matrix(val_baked)
x_test  <- data.matrix(test_baked)

y_train <- as.matrix(train_df$target_class)
y_val   <- as.matrix(val_df$target_class)
y_test  <- as.matrix(test_df$target_class)

# Verify Dimensions
num_classes <- length(unique(y_train))
cat(sprintf("Final Matrix Shapes - X: %d x %d, Y: %d classes detected\n", 
            ncol(x_train), nrow(x_train), num_classes))
...
model <- keras_model_sequential() %>%
  ...
  # UPDATED: Units set to num_classes (7) to match your cleaned data (0-6)
...
# FIX 3: Calculate Class Weights
# ------------------------------------------------------------------------------
# Strategy: "Balanced" weighting. 
# Weight = Total_Samples / (Num_Classes * Count_in_Class)
# This makes the model pay equal attention to rare classes (Defaults) vs common ones.

total_count <- nrow(train_df)
class_counts <- table(train_df$target_class)
...
history <- model %>% fit(
  ...
  class_weight = class_weight_list,  # <--- CRITICAL ADDITION
  epochs = 2000,           
  batch_size = 256,        
  callbacks = callbacks,
  verbose = 1
...
png(here("Saved_Outputs", "training_history_plot.png"), width=800, height=600)
plot(history)
dev.off()
...
# ----------------------------------------------------------
eval_pdf_path <- here("Saved_Outputs", "Detailed_Model_Evaluation_Report.pdf")
...
# B. Calculate MCC (Matthews Correlation Coefficient)
# ----------------------------------------------------------
# We use 'yardstick' because MLmetrics does not have MCC, 
# and yardstick handles Multiclass (0-6) correctly.

if(!require(yardstick)) install.packages("yardstick")
library(yardstick)

# Yardstick requires a data.frame/tibble input
mcc_data <- data.frame(truth = actual_factor, estimate = pred_factor)

# Calculate MCC
mcc_result <- yardstick::mcc(mcc_data, truth = truth, estimate = estimate)
mcc_score <- mcc_result$.estimate

# Print to check
print(paste("Multiclass MCC:", round(mcc_score, 4)))
...
write.csv(results_df, here("Saved_Outputs", "comprehensive_test_predictions.csv"), row.names = FALSE)
