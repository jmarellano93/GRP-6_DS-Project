...
# -----------------
# Load the dataset
path <- "C:/Users/John Arellano/RstudioProjects/GRP-6_DS-Project/DS-Project_Part2_Scripts/Saved_Outputs/Dataset-part-2.csv"
data <- read.csv(path)
...
if(any(data$status == "X")) {
  cat("FAIL: Status 'X' still present.\n")
} else {
  cat("PASS: Status 'X' removed and target encoded.\n")
}
...
write.csv(
  data,
  file = "C:/Users/John Arellano/RstudioProjects/GRP-6_DS-Project/DS-Project_Part2_Scripts/Saved_Outputs/cleaned_dataset.csv",
  row.names = FALSE
)
...
# ------------------------------------------------------------------------------
# 6.5 Correlations & Structural Missingness
# ------------------------------------------------------------------------------
# Correlation Matrix
...
# ==============================================================================
# 6.6 Correlations & Structural Missingness (Extended: Risk Profiling)
# ==============================================================================

# ------------------------------------------------------------------------------
# 6.6.1 Create Behavioral Risk Categories (Safe Diagnostic Copy)
# ------------------------------------------------------------------------------
# We create a temporary dataframe 'df_diagnostics' to perform EDA.
# This prevents modifying the main 'df_clean' dataset used for Neural Network training.

df_diagnostics <- df_clean %>%
  mutate(
    Risk_Category = case_when(
      TARGET %in% c(1, 2) ~ "A_Moderate", # Status 0 (1-29) & 1 (30-59)
      TARGET %in% c(3, 4) ~ "B_Serious",  # Status 2 (60-89) & 3 (90-119)
      TARGET %in% c(5, 6) ~ "C_Extreme",  # Status 4 (120-149) & 5 (>150)
      TRUE ~ "Good_Standing"              # Status C (Paid/Current)
    ),
    # Create Binary Flags for specific analysis
    # UPDATED: 'Is_Delinquent' now captures Moderate (A), Serious (B), and Extreme (C)
    Is_Delinquent = ifelse(Risk_Category != "Good_Standing", 1, 0),
    Is_Extreme = ifelse(Risk_Category == "C_Extreme", 1, 0)
  )

cat("--- Risk Category Distribution (Diagnostic Only) ---\n")
print(prop.table(table(df_diagnostics$Risk_Category)) * 100)

# ------------------------------------------------------------------------------
# 6.6.2 Interaction Heatmap (Focus: Any Delinquency A+B+C)
# ------------------------------------------------------------------------------
# We visualize the rate of ANY delinquency (Moderate + Serious + Extreme)
# to see which segments struggle to pay on time at all.

p_interaction <- df_diagnostics %>%
  group_by(NAME_FAMILY_STATUS, NAME_HOUSING_TYPE) %>%
  summarise(
    Risk_Rate = mean(Is_Delinquent, na.rm=TRUE),
    Count = n(),
    .groups = "drop"
  ) %>%
  filter(Count > 50) %>%
  ...
  scale_fill_viridis_c(option = "magma", name = "Delinquency %", labels = scales::percent) +
  labs(title = "Risk Heatmap: Overall Delinquency Rates (Groups A, B, & C)",
       subtitle = "% of Applicants with any late payments (1+ Days Past Due)",
...
# ------------------------------------------------------------------------------
# 6.6.3 Risk Density Analysis (Violin Plots by Category)
# ------------------------------------------------------------------------------
p_violin <- ggplot(df_diagnostics, aes(x = Risk_Category, y = AMT_INCOME_TOTAL, fill = Risk_Category)) +
  geom_violin(trim = FALSE, alpha = 0.6) +
  scale_y_log10(labels = scales::dollar) +
  scale_fill_brewer(palette = "RdYlGn", direction = -1) +
  labs(title = "Income Distribution by Risk Profile", 
       subtitle = "Do higher earners fall into specific delinquency patterns?",
       x = "Risk Category", y = "Income (Log Scale)") +
  theme_minimal() +
...
# ------------------------------------------------------------------------------
# Calculates rates for A, B, and C groups across key variables

if("Risk_Category" %in% names(df_diagnostics)) {
  ...
  calc_detailed_risk <- function(data, var_name) {
    data %>%
      ...
      summarise(
        Total = n(),
        Mod_Rate = sprintf("%.1f%%", mean(Risk_Category == "A_Moderate") * 100),
        Ser_Rate = sprintf("%.1f%%", mean(Risk_Category == "B_Serious") * 100),
        Ext_Rate = sprintf("%.1f%%", mean(Risk_Category == "C_Extreme") * 100)
      ) %>%
      arrange(desc(Ext_Rate)) # Sort by Extreme Risk
  }
  
  # Bin continuous variables for analysis (Applied to diagnostic copy only)
  ...
  risk_vars <- c("NAME_INCOME_TYPE", "NAME_FAMILY_STATUS", "NAME_HOUSING_TYPE", 
                 "OCCUPATION_TYPE", "EDUCATION_ENCODED", "DAYS_EMPLOYED_BIN")
  
  for(var in risk_vars) {
    if(var %in% names(df_risk)) {
      cat(paste("\n>>> Risk Profile Breakdown by:", var, "\n"))
...
# ------------------------------------------------------------------------------
# We use a Decision Tree to automatically find the rules that lead to 
# "Moderate", "Serious", or "Extreme" buckets.

cat("\n[Analysis] Generating Decision Tree Rules for Risk Profiles...\n")

# Train Tree on the Categorical Risk Target using diagnostic data
tree_model <- rpart(as.factor(Risk_Category) ~ ., 
                    data = df_diagnostics %>% select(-TARGET, -Is_Delinquent, -Is_Extreme), 
                    method = "class", 
                    # Prioritize finding all non-good classes: Good, Mod, Ser, Ext
                    # Adjusted priors to give weight to Moderate (A) as well
                    parms = list(prior = c(0.25, 0.25, 0.25, 0.25)), 
                    control = rpart.control(cp = 0.002, minbucket = 20))

# Extract Rules for "A_Moderate", "B_Serious" and "C_Extreme"
class_levels <- levels(as.factor(df_diagnostics$Risk_Category))

get_tree_rules <- function(target_class_name) {
  # Find column index for the target class
  col_idx <- which(class_levels == target_class_name)
  
  if(length(col_idx) > 0) {
    # Predict on diagnostic data, not clean data
    ...
    profiles <- df_diagnostics %>%
      ...
      summarise(
        Count = n(),
        Target_Prob = mean(Risk_Category == target_class_name),
        Dominant_Job = names(sort(table(OCCUPATION_TYPE), decreasing=T))[1]
      ) %>%
    ...
    if(length(node_ids) > 0) {
      ...
      for(i in 1:length(rules_list)) {
        rule <- rules_list[[i]][-1] # Skip root
        prob <- profiles$Target_Prob[i] * 100
        cat(sprintf("   Profile %d (Prob: %.1f%%): %s\n", i, prob, paste(rule, collapse = " AND ")))
      }
    }
  }
}

# Generate Rules for ALL risk groups
get_tree_rules("A_Moderate")
get_tree_rules("B_Serious")
get_tree_rules("C_Extreme")

# Cleanup of temporary dataframe (Optional, but good practice)
rm(df_diagnostics, tree_model, p_interaction, p_violin)
...
save_model_path <- "C:/Users/John Arellano/RstudioProjects/GRP-6_DS-Project/DS-Project_Part2_Scripts/Saved_Outputs/final_model.keras"
