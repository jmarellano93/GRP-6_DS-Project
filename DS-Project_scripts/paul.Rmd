...
Drop ANY, NONE, OTHER so we can give the levels an order
...
```{r}
# Clean term variable
data$term <- trimws(data$term)                       # remove whitespace
data$term <- ifelse(grepl("36", data$term), "36", "60")  # convert to pure numeric strings
data$term <- factor(data$term,
                    levels = c("60", "36"),          # 60 months is higher risk
                    ordered = TRUE)
...
data$emp_length <- factor(
  data$emp_length,
  levels = c("< 1 year", "1 year", "2 years", "3 years", "4 years",
...
data$home_ownership <- factor(data$home_ownership,
                   levels = c("NONE", "RENT","MORTGAGE", "OWN", "OTHER", "ANY"))
...
Check of OTHER CAN BE DOUND IN ANY and the other way around

Please note: It does not automatically treat the factor as ordinal (ordered from low → high). It will not influence models that require numeric / ordinal meaning unless ordered=TRUE is set. We should consider to use ordered=True for home_ownership (what with na?), for term and verification_status.
...
-   earliest_cr_line_date: There are 25 missing values. These values could be imputed or dropped after further anaylsis.
-   last_credit_pull_d_date: There are 25 missing values. These values could be imputed or dropped after further anaylsis.
...
print(extreme_delinq) # 1'260 rows
```
...
```{r}
cleaned_data <- cleaned_data %>%
  select(-c(id, member_id, url, desc, zip_code, emp_title, policy_code, revol_util, mths_since_last_delinq, mths_since_last_major_derog, delinq_2yrs, mths_since_last_record, pub_rec, loan_amnt, addr_state))
```
...
For this project, the dataset consisting of 734,905 rows is divided into three independent subsets: - Training set (70%) → approx. 514,434 rows - Validation set (15%) → approx. 110,236 rows - Test set (15%) → approx. 110,236 rows This partitioning provides a clear and interpretable workflow while preserving a large training set for model development.

#### Why a simple splitting technique

A single split into training, validation, and test sets offers several practical advantages for this assignment: 1. Clarity and interpretabilit Each dataset serves a distinct purpose: - the training set is used for fitting the models and running cross-validation, - the validation set is used for hyperparameter tuning, feature selection, and model comparison, - the test set is used once at the end to measure final model performance. This transparency aligns well with the educational objective of explaining each modelling decision. 2. Large dataset removes instability concerns With 734k rows, a single split provides sufficiently large training, validation, and test samples. Variance from resampling is not a major concern with this sample size. 3. Supports clear explanation of modelling choices The project requires interpreting and justifying the effects of: - different imputation strategies, - different feature selection decisions, model hyperparameters. A single split makes these stages easy to isolate and explain. 4. Avoids unnecessary complexity A simple split avoids the computational burden and conceptual overhead of nested cross-validation, which is not required for the learning goals of this project.

#### Why no nested loop

In theory, nested cross-validation represents the most statistically rigorous approach. It separates: - the outer loop → unbiased performance estimation - the inner loop → model selection, feature selection, imputation choice, hyperparameter tuning Benefits of Nested CV - Provides the most unbiased estimate of generalization performance - Prevents any contamination between: - model selection - preprocessing selection - performance estimation - Automatically incorporates data-dependent preprocessing (imputers, scalers, feature selection) into the evaluation loop - Ideal for scientific research, regulated environments, or small datasets However, nested CV also has drawbacks in this assignment context Extremely high computational cost Running nested 5×5 CV on 734k rows would require fitting models dozens of times, which is impractical. Makes explainability harder The assignment requires demonstrating: why a specific imputation method was chosen, how features were selected, how hyperparameters influence model behaviour. In nested CV, all these steps are embedded inside opaque inner loops, making it much harder to document and interpret each decision. Overkill for a large dataset Nested CV is most useful when data is scarce. With more than 500k rows in the training set, the benefit over a single validation split is minimal. Not aligned with the pedagogical goal The purpose of this project is to learn the reasoning behind model choices, not to maximize statistical purity at any cost. A simpler split supports clear step-by-step analysis.

### Imputation

Choose imputation method with ONLY Train dataset and make a CV within Train dataset to choose the best one after you fit set imputation method on the entire TRAINING set

fit imputer on train and apply to val fit imputer on train and val and apply to test
...
head(missing_values, 54)  # just to inspect
```
...
```{r}
# Split into train, validation, test (example)
set.seed(123)

# Stratified split: 70% train, 30% temp
train_index <- createDataPartition(cleaned_data$int_rate, p = 0.7, list = FALSE)
train_raw   <- cleaned_data[train_index, ]
temp_raw    <- cleaned_data[-train_index, ]

# Split temp into 50% val, 50% test  -> each 15% of total
val_index <- createDataPartition(temp_raw$int_rate, p = 0.5, list = FALSE)
val_raw   <- temp_raw[val_index, ]
test_raw  <- temp_raw[-val_index, ]
...
```{r}
make_imputed_splits <- function(train_raw, val_raw, test_raw, method) {

  # Separate predictors and target
  predictor_names <- setdiff(names(train_raw), "int_rate")

  x_train <- train_raw[, predictor_names]
  y_train <- train_raw$int_rate

  x_val   <- val_raw[, predictor_names]
  y_val   <- val_raw$int_rate

  x_test  <- test_raw[, predictor_names]
  y_test  <- test_raw$int_rate

  # Fit preProcess (imputer) on TRAIN ONLY
  imputer <- preProcess(x_train, method = method)

  # Apply imputer to all splits (using training params)
  x_train_imp <- predict(imputer, x_train)
  x_val_imp   <- predict(imputer, x_val)
  x_test_imp  <- predict(imputer, x_test)

  # Add target back
  train_imp <- cbind(x_train_imp, target = y_train)
  val_imp   <- cbind(x_val_imp,   target = y_val)
  test_imp  <- cbind(x_test_imp,  target = y_test)
...
```{r}
median_splits <- make_imputed_splits(
  train_raw = train_raw,
  val_raw   = val_raw,
  test_raw  = test_raw,
  method    = "medianImpute"
...
train_median <- median_splits$train
val_median   <- median_splits$val
test_median  <- median_splits$test
```
...
knn_splits <- make_imputed_splits(
  train_raw = train_raw,
  val_raw   = val_raw,
  test_raw  = test_raw,
  method    = c("center", "scale", "knnImpute")
...
train_knn <- knn_splits$train
val_knn   <- knn_splits$val
test_knn  <- knn_splits$test
```
...
pred_val_median <- predict(rf_median, newdata = val_median)
```

```{r}
postResample(pred_val_median, val_median[[target_var]])
```
...
pred_val_knn <- predict(rf_knn, newdata = val_knn)
```


### Transform columns with outliers after imputation

```{r}
cleaned_data$revol_bal_log <- log1p(cleaned_data$revol_bal)
cleaned_data$tot_cur_bal_log <- log1p(cleaned_data$tot_cur_bal)
cleaned_data$total_rev_hi_lim_log <- log1p(cleaned_data$total_rev_hi_lim)
```

Then drop the original column
```{r}
cleaned_data <- cleaned_data %>%
  select(-c(revol_bal, tot_cur_bal, total_rev_hi_lim))
```
```{r}
# We'll save it as "cleaned_data.csv" inside the Intermediate directory.
#output_file_path <- "/Users/Jujou/Documents/Repos/GRP-6_DS-Project/DS-Project_data/Intermediate/cleaned_data.csv"

# Save the data frame to the specified path
