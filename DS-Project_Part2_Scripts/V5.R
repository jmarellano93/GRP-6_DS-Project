...
# --------------------------
set.seed(123)
# Stratified split on the 8-class target to ensure rare classes appear in both sets
split_idx <- createDataPartition(df_model$target_class, p = 0.8, list = FALSE)
train_df <- df_model[split_idx, ]
val_df   <- df_model[-split_idx, ]

# 7.3 Multi-Class Sample Weighting (Dampened)
# -------------------------------------------
# MENTOR UPDATE: We apply a square root to the weights.
# This reduces the massive disparity between rare (Status 6) and common (Status 0) classes,
# helping improve Precision while maintaining acceptable Recall.
...
calc_dampened_weights <- function(target_vec) {
  ...
  # Apply Square Root Damping
  # Example: If raw weight is 600, new weight is sqrt(600) ≈ 24.5
  # If raw weight is 1, new weight is 1.
  dampened_weights <- sqrt(raw_weights)
  
  # Normalize so the mean weight is roughly 1.0 (Optional but good for optimizer stability)
...
cat(">> Multi-Class Weights Calculated (Dampened).\n")
cat("   Sample weights for Status 0 vs Status 5:\n")
# Check specific weights to ensure the gap isn't too large
check_weights <- train_df %>% 
  mutate(w = train_weights) %>% 
  group_by(target_class) %>% 
  summarise(Mean_Weight = mean(w))
print(check_weights)

# 7.4 Recipe Definition (Preprocessing Pipeline)
# ----------------------------------------------
# We treat target_class as a Factor for the recipe logic, but Keras needs Integers.
# We will NOT one-hot encode the target here; we use Sparse Categorical Crossentropy.

rec_obj <- recipe(target_class ~ ., data = train_df) %>%
  step_unknown(all_nominal_predictors()) %>%            
  step_integer(all_nominal_predictors()) %>% 
  # Interaction: Helps model distinguish low-income young people vs low-income seniors
...
for(v in train_tensors$cat_vars) {
  vocab_sizes[[v]] <- max(c(train_baked[[v]], val_baked[[v]]), na.rm = TRUE) + 1
}
...
# ==============================================================================
# SECTION 10: RIGOROUS EVALUATION (Multi-Class)
# ==============================================================================

cat("\n======================================================\n")
cat("   PHASE 5: MULTI-CLASS EVALUATION\n")
cat("======================================================\n")

# 10.1 Generate Predictions
# -------------------------
# 'predict' returns an N x 8 matrix of probabilities
pred_probs <- predict(model, val_tensors$inputs) 

# Convert probabilities to Class IDs (0-7) using argmax
pred_classes <- apply(pred_probs, 1, which.max) - 1 # Minus 1 to convert 1-8 index to 0-7 class

# 10.2 Confusion Matrix
# ---------------------
# Mapping integers back to names for clarity
class_map <- c("0"="Status_C", "1"="Status_X", "2"="Status_0", "3"="Status_1", 
               "4"="Status_2", "5"="Status_3", "6"="Status_4", "7"="Status_5")

actual_factor <- factor(val_tensors$y, levels = 0:7, labels = class_map)
pred_factor   <- factor(pred_classes, levels = 0:7, labels = class_map)

# Detailed Caret Matrix
...
# ----------------------------------
cat("\n>>> Per-Class Statistics (Precision, Recall, F1) <<<\n")
stats <- cm$byClass %>% 
  as.data.frame() %>% 
