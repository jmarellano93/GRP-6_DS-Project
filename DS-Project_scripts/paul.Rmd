...
```{r}
original <- read.csv("~/GRP-6_DS-Project/DS-Project_data/LCdata.csv", 
                 row.names = NULL, 
                 sep = ";",
                 header = TRUE)
...
Furthermore there are other columns to drop, which aren't useful for modelling or have too many missing values. These columns are: - id - member-id - revol_util: can be calculated with revol_bal and total_rev_hi_lim anyway

```{r}
cleaned_data <- cleaned_data %>%
  select(-c(id, member_id, revol_util, mths_since_last_delinq, mths_since_last_major_derog, delinq_2yrs, mths_since_last_record, pub_rec))
```
