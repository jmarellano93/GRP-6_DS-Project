...
#
# MENTOR UPDATE: 
# 1. Implementation of Balanced Batch Oversampling (Section 8).
# 2. Increased Dropout Regularization to prevent overfitting on repeated samples (Section 7).
# ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
# Reason: We require a model capable of learning non-linear relationships.
#
# 7.2: COMPILATION (SGD + MOMENTUM)
# Reason: We have switched from Adam to SGD with Momentum.
#         - SGD often generalizes better than Adam on tabular data.
#         - Momentum (0.9) helps the optimizer "plow through" noisy gradients
#           generated by our stratified sampling batches.
#         - Nesterov: A modification to momentum that "looks ahead" to correct
#           the gradient direction, often improving convergence speed.
...
# MENTOR NOTE: Increased dropout significantly (0.3/0.2) to combat 
# the risk of memorization caused by Oversampling.
model <- keras_model_sequential() %>%
  # Layer 1: High capacity (128)
  layer_dense(units = 128, activation = "relu", input_shape = c(num_features)) %>%
  layer_dropout(0.10) %>% # INCREASED FROM 0.05
  
  # Layer 2: Wide secondary layer
  layer_dense(units = 64, activation = "relu") %>%
  layer_dropout(0.05) %>% # INCREASED FROM 0.025
  
  # Output Layer
  layer_dense(units = num_classes, activation = "softmax") 

# ==============================================================================
# 7.2: Compile Model (SGD + MOMENTUM IMPLEMENTATION)
# ==============================================================================
model %>% compile(
  # MODIFICATION: Replaced optimizer_adam with optimizer_sgd
  # Learning Rate: Increased to 0.01 (SGD needs higher starting rates than Adam)
  # Momentum: Set to 0.9 (Standard for stabilizing updates)
  # Nesterov: TRUE (Helps convergence)
...
model %>% summary()
cat(">> Model compiled with SGD (LR=0.01, Momentum=0.9, Nesterov=TRUE)\n")
...
# Reason: We are implementing "Balanced Batch Sampling".
#         We divide customers into 4 streams (Good, Uncertain, Bad, ExtBad).
#         
#         MENTOR CORRECTION: We now use ARTIFICIAL WEIGHTS (0.25 each) to force
#         the model to see equal amounts of each risk category, regardless of 
#         their actual rarity in the dataset.

# ==============================================================================
# 8.1: Create Risk-Stratified Dataset Pipeline
# ==============================================================================
cat("\n=== CONFIGURING STRATIFIED SAMPLING (Artificial Balancing) ===\n")
...
# 2. Calculate Dynamic Weights (BALANCED OVERSAMPLING)
# MENTOR NOTE: We purposefully ignore the real size (w_good) and enforce equality.
...
# TARGET WEIGHTS: 25% of each stream per batch
target_weights <- c(0.75, 0.10, 0.10, 0.05)
...
cat(">> Dynamic Sampling Weights set to 25% each (Forced Oversampling of Minority).\n")
...
balanced_ds <- sample_from_datasets(
  ...
  weights = target_weights # USES 0.25, NOT NATURAL PROPORTIONS
) %>%
  dataset_batch(128) %>% 
  dataset_prefetch(1)
...
steps_per_epoch <- floor(nrow(x_train) / 128)
