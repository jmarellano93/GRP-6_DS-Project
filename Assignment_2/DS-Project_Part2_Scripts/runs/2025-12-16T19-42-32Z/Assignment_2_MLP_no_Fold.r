...
callback_weightnorm_f1 <- callback_lambda(
  on_epoch_end = function(epoch, logs) {
    ...
    # Run only every 250 epochs (0, 250, 500, ...)
    if (epoch %% 250 == 0) {
      
      # 1. Weight L2 Norm (The "Pressure" Gauge)
      weight_norm <- sum(sapply(model$trainable_weights, function(w) {
        as.numeric(tf$norm(w))
      }))
      
      # 2. Validation Metrics (The "Result")
      ...
      # 3. Print to Console
      cat(sprintf(
        "\n[Epoch %d] Weight Norm: %.2f | Val Macro F1: %.4f | Val Bal Acc: %.4f\n", 
        epoch, weight_norm, macro_f1, bal_acc
      ))
    }
  }
)
