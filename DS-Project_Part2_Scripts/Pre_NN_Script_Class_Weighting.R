# ==============================================================================
# REPORT VERSION 4: CLASS WEIGHTING STRATEGY
# FILENAME: Pre_NN_Script_Class_Weighting.R
# ==============================================================================
...
# ==============================================================================
# Module 5: Advanced Preprocessing & Data Splitting (WEIGHTED CLASSES)
# ==============================================================================
# METHODOLOGY: 
# In this version, we perform NO SAMPLING. We preserve the natural imbalance.
# Instead, we will calculate Class Weights in Module 6 to penalize the model 
# heavily for missing the minority class during training.
#
# ADVANTAGE: Uses all data (no loss), no synthetic noise (no overfitting).
# DISADVANTAGE: Requires careful passing of weights to Keras fit().
...
# ==============================================================================
# STRATEGY: PURE PREPROCESSING (NO SAMPLING)
# ==============================================================================

final_recipe <- base_recipe %>%
  # NOTE: No step_smote, step_upsample, or step_downsample here.
  
  # Step A: One-Hot Encoding
  step_dummy(all_nominal_predictors(), one_hot = TRUE) %>%
  
  # Step B: Cleanup
  step_zv(all_predictors()) %>%
  
  # Step C: Min-Max Scaling 
  step_range(all_numeric_predictors(), min = 0, max = 1)

strategy_name <- "Class Weighting (No Sampling)"
...
")
cat("Target Distribution (Train - SHOULD BE IMBALANCED):
")
print(table(train_processed$TARGET))

cat(">> QA PASS: Preprocessing pipeline (No Sampling) verified.
")
cat("
--- Module 5 Complete ---
")
...
# ==============================================================================
# Module 6: Final Data Formatting & Export + WEIGHT CALCULATION
# ==============================================================================
cat("
================================================================
")
cat(" MODULE 6: DATA EXPORT & CLASS WEIGHT CALCULATION
")
cat("================================================================
")
...
# --- 6.3 CRITICAL: CALCULATE CLASS WEIGHTS ---
# Formula: Weight(Class) = Total_Samples / (Num_Classes * Count(Class))
# This balances the loss contribution.

counts <- table(train_processed$TARGET)
total_samples <- sum(counts)
num_classes <- 2

# Calculate weights
w0 <- total_samples / (num_classes * counts) # Weight for class 0 (Majority) -> approx 0.5
w1 <- total_samples / (num_classes * counts) # Weight for class 1 (Minority) -> High value

# Create the Named List required by Keras
# Note: Keras expects keys as strings "0", "1"
...
# --- 6.4 Save to Disk ---
output_dir <- "processed_data_weighted"
...
# Save the weights specifically for the NN script to load
saveRDS(class_weights_list, file.path(output_dir, "class_weights.rds"))

cat(">> SUCCESS: Data and Weights are ready for Neural Network Training.
")
cat(">> INSTRUCTION: In your NN script, load 'class_weights.rds' and pass it 
")
cat(">> to model %>% fit(..., class_weight = loaded_weights).
")
