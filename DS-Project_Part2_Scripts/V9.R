...
req_pkgs <- c("tidyverse",    # Collection of packages (ggplot2, dplyr, etc.)
              ...
              "recipes",      # Data Preprocessing (step_dummy, step_impute)
              "MLmetrics",    # Advanced Metrics (MCC, LogLoss)
...
# ==============================================================================
# 3.3: Age and Employment Logic (CORRECTED)
# ==============================================================================
...
# 3.3.4: PREVENTING FEATURE COMPRESSION
# Old Logic: Replaced 365243 with 21915.
# New Logic: Replace with 0. 
# Why? This allows working people (0-40 years) to occupy the full variance 
# of the feature after scaling. The 'Pensioner' status is already captured 
# by NAME_INCOME_TYPE.
...
# Check if max employment is reasonable (should be < 60 years now, not 1000)
reasonable_emp  <- max(data$ACTIVE_EMPLOYMENT_YEARS) < 100

cat(sprintf("Negative Age: %s. Sentinel Remains: %s. Reasonable Max Emp: %s.\n", 
            neg_age, sentinel_remain, reasonable_emp))

if (!neg_age && !sentinel_remain && reasonable_emp) {
  cat("PASS: Age positive, sentinels removed, feature compression avoided.\n")
} else {
...
# ------------------------------------------------------------------------------
# SECTION 8: MODEL TRAINING & OPTIMIZATION
# ------------------------------------------------------------------------------

# SCRIPT ARCHITECTURE SUMMARY: SECTION 8
#
# 8.1: ROBUST CLASS WEIGHTING
# Reason: The dataset is highly imbalanced (Defaults are rare). We calculate
#       inverse frequency weights so the model is penalized heavily for missing
#       a default, but lightly for missing a paid loan. 
#       Includes a mathematical patch (epsilon) to prevent division-by-zero errors.
#
# 8.2: TRAINING CALLBACKS
# Reason: Prevents Overfitting.
#       - Early Stopping: Halts training if Validation Accuracy stops improving.
#       - Reduce LR: Lowers learning rate when the model hits a plateau.
...
weights_vec <- total_count / (length(possible_classes) * (class_counts + 1e-6))
class_weight_list <- as.list(weights_vec)
names(class_weight_list) <- possible_classes

cat(">> Robust Class Weights (Penalizing missed Defaults):\n")
...
history <- model %>% fit(
  ...
  epochs = 4000, batch_size = 256, callbacks = callbacks, verbose = 1
)
