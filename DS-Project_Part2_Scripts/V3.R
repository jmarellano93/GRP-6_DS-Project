...
# KEY FINDINGS
# ------------------------------------------------------------------------------
# 1. Anomalies: 365243 in DAYS_EMPLOYED is a sentinel value for Pensioners.
# 2. Distribution: Income is Pareto distributed (skew ~8.48). Requires Log transform before Min-Max scaling.
# 3. Missingness: Occupation is MNAR. We must use a dedicated 'Unknown' or 'Retired' category, not generic imputation.
...
# ==============================================================================
# Module 5: Data Preparation (Refined Feature Engineering & Interaction Logic)
# ==============================================================================
# PURPOSE: Implement Log-stabilization and multiplicative risk interactions 
# per the Expert Optimization Strategy (Part II, 2.2).

cat("\n================================================================\n")
cat(" MODULE 5: REFINED PREPROCESSING & INTERACTION ENGINEERING\n")
cat("================================================================\n")

set.seed(123)
df_clean$TARGET <- as.factor(df_clean$TARGET)

# 1. Stratified Splitting to preserve 11% minority class ratio
...
# 2. Define High-Turnover/Toxic Categories for Interaction Features
toxic_jobs <- c("Laborers", "Sales staff", "Drivers", "Cooking staff", 
                "Security staff", "Waiters/barmen staff", "Low-skill Laborers")

# 3. Construct Refined Recipe Pipeline
base_recipe <- recipe(TARGET ~ ., data = train_raw) %>%
  update_role(any_of("ID"), new_role = "id") %>%
  step_rm(any_of("CNT_FAM_MEMBERS")) %>%
  
  # A. Handling Skewness: Logarithmic stabilization of income (Part I, 1.2)
  step_log(AMT_INCOME_TOTAL, offset = 1, base = 10) %>%
  
  # B. Imputation Logic
  step_impute_median(all_numeric_predictors()) %>%
  step_unknown(all_nominal_predictors()) %>%
  
  # C. Multiplicative Interaction Engineering (Part IV, 4.1)
  step_mutate(
    # RISK_SCORE_INTERACTION: Financial burden compounded by precarious housing
    RISK_INT_FACTOR = case_when(
      EMPLOYMENT_STATUS_FLAG == "Working" & NAME_HOUSING_TYPE == "Rented apartment" ~ 1.5,
      NAME_FAMILY_STATUS %in% c("Separated", "Single / not married") & CNT_CHILDREN == 0 ~ 1.2,
      TRUE ~ 1.0
    ),
    
    # EMPLOYMENT_VULNERABILITY: Ratio of duration to sector volatility
    # Note: Normalizing DAYS_EMPLOYED temporarily within the mutate for calculation
    EMP_STABILITY = (DAYS_EMPLOYED / max(DAYS_EMPLOYED, na.rm=TRUE)) * (1 + (CNT_CHILDREN * 0.1)),
    EMP_VULNERABILITY = if_else(
      EMPLOYMENT_STATUS_FLAG == "Working" & OCCUPATION_TYPE %in% toxic_jobs,
      1 / (EMP_STABILITY + 0.01), 
      0
    )
  ) %>%
  step_rm(EMP_STABILITY) %>% # Remove temporary helper column
  
  # D. Final Formatting for Keras
  step_dummy(all_nominal_predictors(), one_hot = TRUE) %>%
  step_zv(all_predictors()) %>%
  step_range(all_numeric_predictors(), min = 0, max = 1)

# 4. Prepare and Bake Data
...
# 5. Optimized Matrix Extraction Function
# Bridges engineered features to the Surgical Sample Weighting in Module 6
process_matrix <- function(df) {
  # Capture engineered risk flags for weight calculation
  risk_flags <- df %>% select(starts_with("RISK_"), starts_with("EMP_VULNERABILITY"))
  
  list(
    x = as.matrix(df %>% select(-TARGET) %>% mutate(across(everything(), as.numeric))),
    y = as.integer(df$TARGET) - 1,
    flags = risk_flags # Crucial for Module 6's row-wise weighting logic
  )
}

train_keras <- process_matrix(train_processed)
val_keras   <- process_matrix(val_processed)
test_keras  <- process_matrix(test_processed)

# Final Clean-up and Handover to Module 6
cat(">> MODULE 5 COMPLETE: Interaction features engineered.\n")
cat(">> Columns Processed:", ncol(train_keras$x), "\n")
...
# --- REVISED MODULE 7 SNIPPET ---
hyper_grid <- expand.grid(
  Units_L1 = c(128, 256),
  Dropout  = c(0.3, 0.4), # Increased for robustness (Part III, 3.2)
  LR       = c(0.001, 0.0001),
  # Increased multiplier range from 2.0 to 5.0-10.0x (Part II, 2.1)
  Risk_Multiplier = c(5.0, 7.5, 10.0) #
)
...
# --- REVISED INSERTION FOR MODULE 8 (Calibration) ---
# 1. Generate probabilities
val_probs <- as.numeric(final_model$predict(val_x_np, batch_size = 2048L))

# 2. Perform Grid Search for T_Optimal
threshold_calibration <- tibble(T = seq(0.05, 0.50, by = 0.01)) %>%
  rowwise() %>%
  mutate(
    # Convert the list output into a dataframe-compatible format
    res = list(as_tibble(calculate_comprehensive_metrics(val_data$y, val_probs, threshold = T)))
  ) %>%
  ungroup() %>% 
  unpack(res) # This will correctly expand Accuracy, Recall, MCC, etc. into columns

# 3. Identify the row that maximizes Recall while Accuracy >= 0.90
final_calibrated_config <- threshold_calibration %>%
  filter(Accuracy >= 0.90) %>%
  arrange(desc(Recall)) %>%
  slice(1)

# Safety check: if no threshold reached 90%, take the highest Accuracy available
if(nrow(final_calibrated_config) == 0){
  cat(">> WARNING: 90% Accuracy not met. Selecting highest accuracy threshold.\n")
  final_calibrated_config <- threshold_calibration %>%
    arrange(desc(Accuracy)) %>%
    slice(1)
}

T_Optimal <- final_calibrated_config$T

cat(sprintf("Optimal Deployment Threshold: %.2f\n", T_Optimal))
cat(sprintf("Validated Recall at Target Accuracy: %.4f\n", final_calibrated_config$Recall))
...
if(exists("final_history")) {
  ...
  p_loss <- p_loss + labs(caption = "Note: Training loss is weighted; Validation loss is unweighted.")
