...
callback_weightnorm_f1 <- callback_lambda(
  on_epoch_end = function(epoch, logs) {
    ...
    # Check every 50 epochs (Index 0, 50, 100...)
    # This corresponds to Display Epoch 1, 51, 101...
    if (epoch %% 50 == 0) {
      
      # 1. Calculate Weight Norm
      weight_norm <- tryCatch({
        sum(sapply(model$trainable_weights, function(w) {
          as.numeric(tf$norm(w))
        }))
      }, error = function(e) -1) # Return -1 if tf$norm fails
      ...
      # 3. FORCE PRINTING (Black text with clear separators)
      cat("\n\n") 
      cat("======================================================\n")
      cat(sprintf("EPOCH %d DIAGNOSTICS:\n", epoch + 1))
      cat(sprintf(" > Weight Norm:   %.2f (Goal: Dropping)\n", weight_norm))
      cat(sprintf(" > Val Macro F1:  %.4f (Goal: Rising)\n", macro_f1))
      cat(sprintf(" > Val Bal Acc:   %.4f\n", bal_acc))
      cat("======================================================\n")
      cat("\n")
      
      # Force R to push the text to the console immediately
      flush.console()
    }
  }
)
history <- model %>% fit(
  ...
  callbacks = list(callback_weightnorm_f1) # Added Callback Here
)
