...
cat("\n--- Performing Row Deduplication ---\n")
initial_rows <- nrow(data)
data <- data %>% distinct()
final_rows <- nrow(data)
dropped_rows <- initial_rows - final_rows

cat(sprintf(">> Deduplication Complete: %d duplicate rows removed.\n", dropped_rows))
cat(sprintf(">> New Row Count: %d\n", final_rows))
...
# B. Collinearity Elimination (caret::findCorrelation)
